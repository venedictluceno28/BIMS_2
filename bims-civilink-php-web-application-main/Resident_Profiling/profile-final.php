...
$user_id = (isset($_GET['id']) && $_GET['id'] !== '') ? intval($_GET['id']) : intval($row['max']);
...
<body style="font-family: calibri; font-size: 18px; background: linear-gradient(to right, #03AF34 0%, #FFF84A 100%) !important; padding: 25px;">
  ...
  <?php
  // Ensure $db_res_purok is initialized to avoid undefined variable warning
  if (!isset($db_res_purok)) {
    $db_res_purok = '';
  }
  $db_res_pur = '';
  if (!empty($db_res_purok)) {
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if (!empty($row['res_Img'])) {
        $imageData = base64_encode($row['res_Img']);
        $src = 'data:image/jpeg;base64,' . $imageData;
    } else {
        $src = 'image.png'; // fallback image path
    }

    echo '<img src="' . $src . '" height="200" width="200" class="img-circle img-responsive"/>';
...
?>
                ...
                <div class="col-sm-7">
                  <?php
                  // Safely display address parts, avoid undefined variable warnings
                  $address_parts = array();

                  if (!empty($db_res_unit)) $address_parts[] = $db_res_unit;
                  if (!empty($db_res_build)) $address_parts[] = $db_res_build;
                  if (!empty($db_res_lot)) $address_parts[] = "L" . $db_res_lot;
                  if (!empty($db_res_block)) $address_parts[] = "BLK" . $db_res_block;
                  if (!empty($db_res_phase)) $address_parts[] = "PH" . $db_res_phase;
                  if (!empty($db_res_house)) $address_parts[] = $db_res_house;
                  if (!empty($db_res_street)) $address_parts[] = $db_res_street . " Street";
                  if (!empty($db_res_sub)) $address_parts[] = $db_res_sub;
                  if (!empty($db_res_pur)) $address_parts[] = $db_res_pur;
